...
// Direct-inviter income for the wallet-top-up portion of an approved
// subscription cash-in (merchant or crypto) — same 10% mechanism and
// rate as StrategyReferralIncomeService, just triggered at admin
// approval / webhook confirmation instead of strategy activation.
// Kept as its own service rather than folded into
// StrategyReferralIncomeService because that one is keyed to
// strategy_activation_id specifically; this one has no such row.

final class CashinReferralIncomeService
{
    /**
     * Credits the direct inviter once for the wallet-top-up portion of a
     * cash-in. $sourceType distinguishes the cash-in rail (e.g.
     * "merchant-cashin" or "crypto-payment") purely for the reference
     * key / audit trail — the rate and mechanics are identical either way.
     *
    ...
    public function credit(
        ...
        string $sourceType,
        int $sourceId,
        string|int|float $topUpAmount,
        ?CarbonInterface $occurredAt = null,
    ): ?array {
        $baseAmount = BigDecimal::of((string) $topUpAmount)
        ...
        $transactionId = DB::table('wallet_transactions')->insertGetId([
            ...
            'event_type' => 'CASHIN_REFERRAL_INCOME',
            'direction' => 'CREDIT',
            'asset' => $account->base_asset,
            ...
            'description' => 'Direct inviter income credited from a referral\'s wallet top-up.',
